...
function xmldb_block_semsort_upgrade($oldversion, $block) {
    global $DB;

    if ($oldversion < 2013010904) {
        $settings = $DB->get_records_list('config', 'name', array('semsort_wintermonths',
            'semsort_sortcourses'));

        $dataobject = new stdClass;
        $dataobject->plugin = 'blocks/semsort';
        ...
        upgrade_block_savepoint(true, 2013010904, 'semsort');
    }

    if ($oldversion < 2014033101) {
        $settings = $DB->get_records('config_plugins', array('plugin' => 'blocks/semsort'));
        foreach ($settings as $id => $setting) {
            $setting->plugin = 'block_semsort';
            $DB->update_record('config_plugins', $setting, true);
        }
        upgrade_block_savepoint(true, 2014033101, 'semsort');
    }
