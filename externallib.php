...
class block_semsort_external extends external_api {
    ...
    public static function get_courses($userid) {
        ...
        $courses = block_semsort_fill_course_semester($courses, $config);
    ...
    public static function get_coursedetails($userid, $courseid) {
        ...
        $result = array();
        $output = $PAGE->get_renderer('block_semsort');
        $expandedevents = block_semsort_get_courses_events($courses, $output);
        if (isset($expandedevents[$courseid]) && !empty($expandedevents[$courseid]->events)) {
            ...
            foreach ($expandedevents[$courseid]->events as $event) {
                ...
                $mod->modinfo = $output->render_from_template('block_semsort/course-event-list-item-clean', $event);
                $result[] = $mod;
            }
        }
